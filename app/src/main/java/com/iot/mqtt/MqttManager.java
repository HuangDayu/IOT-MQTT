...
public class MqttManager {
    ...
    private static MqttAndroidClient mqttAndroidClient;// mqtt客户端
    private static MqttConnectOptions mqttConectOption;// mqtt设置
...
//        }
    ...
    public void publish(String topic, String payload) {
        ...
        if (mqttAndroidClient.isConnected() && null != mqttAndroidClient) {
            try {
                ...
                mainActivity.printlnUpTextView(payload);
    ...
    private static PayloadRoot packPayloadRoot(String namespace,String name,String deviceId ) {
        PayloadRoot root = new PayloadRoot();
        root.setMessageType("Request");
        root.setClientId(SettingActivity.getProduct_ID());
        root.setTopic(SettingActivity.getEquipment_Topic());
        root.setNamespace(namespace);
        root.setName(name);
        root.setDeviceId(deviceId);
        root.setDeviceType("light");
        //UUID.randomUUID().toString().split("-")[0]
        //new MD5Generator().toString()
        root.setMessageId(UUID.randomUUID().toString());
        root.setPayloadVersion("1");
        root.setPublishTime(getData());
        root.setRemarks("Android");
        return root;
    }
     ...
     */
    public void publishMqttPayloadJson(String json_key, String json_volue) {
        if (mqttAndroidClient.isConnected() && null != mqttAndroidClient) {
            String jsonPayload = new JSONObject().toJSONString(MqttManager.packPayloadRoot("Control",json_key,json_volue));
            publish(SettingActivity.getEquipment_Topic(), jsonPayload);
        } else {
            mainActivity.setEvenEnum(EvenEnum.MQTT_CONNECT_FALSE);
        }
    }
     ...
     * @param json_volue json格式数据报尾
     */
    public void sendData_Json1(String json_key, String json_volue) {
        if (mqttAndroidClient.isConnected() && null != mqttAndroidClient) {
